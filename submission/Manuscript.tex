...
\textbf{Word count, body.} 2721\\
\textbf{Tables.} 5 \quad \textbf{Figures.} 4\\
...
published cohorts \cite{ref31}. The Ethicus-2 investigators found comparable
variation across 199 units in 36 countries \cite{ref32}. Variation has been
documented between countries, between regions and between units within a single
country \cite{ref31,ref36,ref37}. Two academic centers at opposite extremes of
treatment intensity differed in decision making norms rather than in patients
\cite{ref34}. Hospital level measures of end of life treatment intensity have
been developed and validated for precisely this reason \cite{ref33}. Hart and
...
after risk adjustment \cite{ref30}. Measurable center characteristics did not
explain that variation \cite{ref30}.
...
\cite{ref24}, and in 36.6\% of a medical intensive care cohort when orders for
life sustaining treatment were included \cite{ref22}. Timing varies, with roughly
a quarter of orders written within 48 hours of admission and the remainder later
\cite{ref25}. These orders carry substantial prognostic weight. Mortality among
patients with such orders is several times that of patients without them
\cite{ref22,ref26}, and the association persists after propensity matching
\cite{ref29}. Placing an order also changes subsequent care, including the
...
admissions to 104 units during 2002 and 2003 \cite{ref1}. Its covariates describe physiology, age, comorbidity and admission
diagnosis \cite{ref1}. Comparable models including SAPS 3 share this structure
\cite{ref10,ref11}. No covariate in any of them represents a decision to withhold
...
have been required \cite{ref6,ref7}. Performance also differs across ethnic groups within the same health
system \cite{ref9}. Calibration is generally the first property to fail while
discrimination is preserved \cite{ref6}.

Standardized mortality ratios derived from these scores are the accepted currency
of intensive care benchmarking \cite{ref12}. Units are compared and ranked on
observed deaths divided by predicted deaths, and in some systems reimbursed on
that basis \cite{ref12}. The ratio is assumed to isolate quality of care
once case mix is accounted for \cite{ref12}. Several lines of evidence suggest
the assumption is fragile. Glance and colleagues showed that the choice of
scoring system determines which units are labeled outliers \cite{ref12}. Kramer
...
the standardized mortality ratio in only 45\% of units \cite{ref13}. Admission
patterns alone alter mortality based performance measures \cite{ref14}. Risk
...
one \cite{ref15}. Model misspecification introduces bias into the resulting
quality measures \cite{ref19}, and alternative modeling strategies produce
materially different unit level estimates \cite{ref16,ref17}.
...
\cite{ref35}. Feng and Dubin identified variables associated with incorrect
...
calibration \cite{ref47}. No study has measured the consequence for risk adjusted
...
program during 2014 and 2015 \cite{ref46}. The database carries vital signs,
laboratory values, care plan documentation, severity scores and outcomes
\cite{ref46}, and has been characterized in detail elsewhere \cite{ref49}. It is
deidentified and was accessed under a data use agreement following completion of
human subjects research training \cite{ref46}. Institutional review board
...
\subsection*{Cohort}
We included adult unit stays with a valid APACHE IVa predicted hospital mortality
...
comprising 135{,}325 admissions. Figure~\ref{fig:flow} shows the derivation.
...
limitation orders \cite{ref25} and limits the degree to which the exposure
...
observed risk and the predicted risk \cite{ref39,ref40}. Standard errors were
...
similarly \cite{ref48}. Discrimination was assessed on a random 30\% holdout of
...
recommended practice \cite{ref56}. Balance was assessed by standardized mean
differences against a threshold of 0.1 \cite{ref56}. We then compared limitation
...
\cite{ref41}. We additionally evaluated two simpler strategies, excluding
...
Brier score from 0.0621 to 0.0579 (Table~\ref{tab:discrim}). Adding malignancy
...
substantially (Table~\ref{tab:rank}). The median absolute rank shift was 7.5
...
changed outlier classification against exact Poisson limits \cite{ref41}. Four
...
that stratum was nine times larger than elsewhere \cite{ref39}. The model is not
...
\cite{ref30}. Our design equalized the covariate distributions rather than
adjusting for them \cite{ref56}, and roughly four fifths of the practice
...
\cite{ref53}, and integration of palliative care into critical care alters both
process and outcome measures \cite{ref54,ref55}. Units therefore differ in
limitation rate partly because they differ in service configuration, which is a
property of the unit rather than of its patients \cite{ref34}.
...
55\% of units \cite{ref13}. Glance and colleagues reached a similar conclusion
comparing scoring systems \cite{ref12}, and admission patterns alone have been
shown to shift these measures \cite{ref14}. We find that a single variable
...
omission penalizes institutions serving different populations \cite{ref21}. The
parallel is exact. Misspecification of the adjustment model is itself a
recognized source of bias in quality measurement \cite{ref19}. Our
...
APACHE IVa \cite{ref13}. Adding one narrow exclusion is feasible within existing practice.
...
program \cite{ref46}. Limitation status is partly a consequence of prognosis.
...
\cite{ref27}. Adjusting for a variable partly under unit control carries the
...
\cite{ref56}. Cancer ascertainment relied on APACHE comorbidity flags and
captured metastatic solid tumor and hematologic malignancy only \cite{ref48}.
...
The eICU Collaborative Research Database is available to credentialed
investigators from PhysioNet. Analysis code is available at [repository URL].
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.42\textwidth]{figures_pdf/Figure1_flow.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures_pdf/Figure2_model_performance.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures_pdf/Figure3_unit_variation.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures_pdf/Figure4_benchmarking.pdf}
